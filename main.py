...
def main(args):
    ...
    if args.eval_only:
        ...
        learnable_params.extend(model.prompt_learner.parameters())

        # Add soft prompt parameters from audio encoder
        # Mode 1: gamma, beta
        # Also includes logmel_gamma, logmel_beta (freq_mode)
        print("\n=== ASPL: Audio Encoder Learnable Parameters ===")
        for name, param in model.audio_encoder.named_parameters():
            if 'gamma' in name or 'beta' in name or 'film' in name:
                learnable_params.append(param)
                print(f"Audio encoder learnable param: {name}, shape={param.shape}")
        print("=" * 50 + "\n")
        
        # num_params = sum(p.numel() for p in learnable_params)
        ...
        trainer.run_training(model, train_dataloader, test_dataloader, optimizer, criterion, device, epochs=args.n_epochs, args=args, learnable_params=learnable_params)
