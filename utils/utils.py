...
def get_args():
    ...
    # ASPL added
    parser.add_argument('--aspl', type=int, default=0, choices=[0, 1, 2], help='ASPL mode: 0 disables audio-side prompt learning, 1 enables ASPL, 2 enables ASPL+ (default: 0)')
...
def save_best_results(seed, best_results, json_file_path):
    """Save best test results to a separate file"""
    # Create a best_results file path from the json_file_path
    best_results_path = json_file_path.replace('.json', '_best.json')
    ...
    with open(best_results_path, "r") as file:
        best_json = json.load(file)
    
    # Save best results with seed as key
    best_json[f"seed_{seed}"] = {
        "epoch": best_results['epoch'],
        "accuracy": f"{best_results['accuracy']:0.6f}",
        "f1_score": f"{best_results['f1_score']:0.6f}",
        "precision": f"{best_results['precision']:0.6f}",
        "recall": f"{best_results['recall']:0.6f}",
        "test_loss": f"{best_results['test_loss']:0.6f}"
    }
    
    # Convert numeric strings back to floats (except epoch)
    for metric in best_json[f"seed_{seed}"].keys():
        if metric != 'epoch':
            best_json[f"seed_{seed}"][metric] = float(best_json[f"seed_{seed}"][metric])
    
    # Save updated best results
    with open(best_results_path, "w") as file:
        json.dump(best_json, file, indent=2)
...
def setup_logging(args):
    aspl_tag = f'aspl{args.aspl}' if args.aspl else 'base'
    log_dir = os.path.join('logs', f'{args.model_name}_{aspl_tag}_{args.num_shots}')
